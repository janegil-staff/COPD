...
function Divider() {
  return (
    <div
      style={{ height: 1, background: "rgba(38,142,134,0.1)", margin: "4px 0" }}
    />
  );
}
...
export default function Sidebar({ t, patient, selectedRecord, show }) {
  ...
  const hasVisibleRecordData =
    selectedRecord &&
    (show.catScore ||
      (show.exacerbation &&
        (selectedRecord.moderateExacerbations ||
          selectedRecord.seriousExacerbations)) ||
    ...
    <div className="flex flex-col">
      ...
      {selectedRecord ? (
        hasVisibleRecordData ? (
          <>
            <SectionHeader>
              {t.catScore} · {selectedRecord.date}
            </SectionHeader>
            ...
            {show.note && selectedRecord.note?.trim() && (
              ...
              >
                ...
                <p className="text-xs" style={{ color: "#6d5a9a" }}>
                  {selectedRecord.note}
                </p>
              </div>
            )}
                ...
                {show.activity && selectedRecord.physicalActivity > 0 && (
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ color: "#268E86" }}
                    >
                      {selectedRecord.physicalActivity} min
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="px-5 py-6 text-center">
            ...
            <p className="text-xs mt-1" style={{ color: "#c8dedd" }}>
              {t.noData}
            </p>
          </div>
        )
            ...
            {patient.userMedicines.map((um) => (
              <div key={um.medicineId} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs" style={{ color: "#7a9a98" }}>
                    {um.medicine?.type === 2 ? t.asNeeded : t.daily} · {t.from}{" "}
                    {um.startedUsage}
                    {um.stoppedUsage
                      ? ` · ${t.stopped} ${um.stoppedUsage}`
                      : ""}
                  </p>
                </div>
                {!um.stoppedUsage && (
          ...
          <div className="px-5 pb-4 space-y-2">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                ...
                style={{
                  background: `${gad7Color}18`,
                  color: gad7Color,
                  border: `1px solid ${gad7Color}40`,
                }}
              >
                {gad7Sum} · {gad7Level}
              </span>
            ...
            {GAD7_KEYS.map((key) => (
              <div key={key} className="flex items-center gap-2">
                ...
                >
                  {t[`gad7${key.charAt(0).toUpperCase()}${key.slice(1)}`] ??
                    key}
                </span>
                <div
                  className="flex-1 h-1.5 rounded-full overflow-hidden"
      ...
      {medSat?.medicines?.length > 0 && (
        <>
          <SectionHeader>
            {t.medicineSatisfaction} · {medSat.date}
          </SectionHeader>
