...
export default function Sidebar({ t, patient, selectedRecord }) {
    ...
    <div className="flex flex-col">
      ...
      {selectedRecord ? (
        <>
          ...
          {(selectedRecord.moderateExacerbations || selectedRecord.seriousExacerbations) && (
          ...
          {(selectedRecord.weight || selectedRecord.physicalActivity > 0) && (
            <div className="mx-5 mb-4 flex gap-2">
              ...
              {selectedRecord.physicalActivity > 0 && (
                <div className="flex-1 px-3 py-2 rounded-xl text-center" style={{ background: "rgba(38,142,134,0.06)", border: "1px solid rgba(38,142,134,0.15)" }}>
                  <p className="text-xs" style={{ color: "#7a9a98" }}>{t.physicalActivity}</p>
                  <p className="text-sm font-bold" style={{ color: "#268E86" }}>{selectedRecord.physicalActivity} min</p>
                </div>
              )}
            </div>
          )}
      ...
      {/* Medicines */}
      {patient.userMedicines?.length > 0 && (
        <>
          ...
          <div className="px-5 pb-4 space-y-3">
            {patient.userMedicines.map((um) => (
              <div key={um.medicineId} className="flex items-center gap-3">
                {um.medicine?.image && (
                  <img src={um.medicine.image} alt={um.medicine.name} className="w-9 h-9 object-contain rounded-lg"
                ...
                {!um.stoppedUsage && <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: "#0f8a6a" }} />}
              </div>
            ))}
          </div>
          <Divider />
      ...
      {gad7 && (
        <>
          ...
          <div className="px-5 pb-4 space-y-2">
            ...
            {GAD7_KEYS.map((key) => (
              <div key={key} className="flex items-center gap-2">
                <span className="text-xs shrink-0" style={{ color: "#7a9a98", width: 148 }}>{t[`gad7${key.charAt(0).toUpperCase()}${key.slice(1)}`] ?? key}</span>
                <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(38,142,134,0.1)" }}>
