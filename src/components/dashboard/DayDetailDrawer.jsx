  ...
  return (
    <div className="p-6">
      ...
      {show.exacerbation && (record.moderateExacerbations || record.seriousExacerbations) && (
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl mb-4" style={{ background: "#fff0f0", border: "1px solid #fca5a5" }}>
          <span>⚠️</span>
      ...
      {(show.weight || show.activity) && (
        <div className="flex gap-3 mb-5">
          {show.weight && record.weight != null && (
            <div className="flex-1 px-3 py-2.5 rounded-xl text-center" style={{ background: "rgba(38,142,134,0.06)", border: "1px solid rgba(38,142,134,0.15)" }}>
      ...
      {show.medicine && usedMedicines.length > 0 && (
        <>
          ...
          <div className="space-y-2 mb-5">
            {usedMedicines.map((m) => (
              <div key={m.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ background: "#f0f9ff", border: "1px solid #bae6fd" }}>
                {m.image && (
                  <img src={m.image} alt={m.name} className="w-8 h-8 object-contain rounded-lg"
                ...
                {m.times != null && (
                  <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: "#e0f2fe", color: "#0369a1" }}>
                    {m.times}{t.timesUsed}
                  </span>
                )}
              </div>
      ...
      {show.note && record.note?.trim() && (
        <>
          <p className="text-xs font-semibold tracking-widest uppercase mb-2" style={{ color: "#a0b8b6" }}>{t.note}</p>
...
export default function DayDetailDrawer({ t, open, onClose, record, medicines, userMedicines, show }) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <>
      {/* Mobile — slide up */}
      <div className="fixed inset-0 z-40 lg:hidden" style={backdropStyle} onClick={onClose} />
      <div
      ...
      >
        <div className="w-10 h-1 rounded-full mx-auto mt-3 mb-1" style={{ background: "rgba(38,142,134,0.25)" }} />
        <DrawerContent t={t} record={record} catColor={catColor} usedMedicines={usedMedicines} onClose={onClose} show={show} />
      </div>

      {/* Desktop — centred modal */}
      <div className="hidden lg:block fixed inset-0 z-40" style={backdropStyle} onClick={onClose} />
      <div
