...
const CAT_KEYS = [
  "cat8Cough","cat8Phlegm","cat8ChestTightness","cat8Breathlessness",
...
function ScoreBar({ value, max = 5 }) {
  return (
    <div className="flex items-center gap-1.5">
      <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(38,142,134,0.12)", minWidth: 48 }}>
        <div className="h-full rounded-full" style={{ width: `${(value / max) * 100}%`, background: BAR_COLOR(value) }} />
      </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: expanded ? "0 4px 24px rgba(38,142,134,0.1)" : "0 1px 4px rgba(0,0,0,0.04)",
      }}
    >
      ...
      >
        ...
        <div className="flex-shrink-0 w-28">
          <p className="text-sm font-bold" style={{ color: "#1a3a38", fontFamily: "'Playfair Display', Georgia, serif" }}>
            {record.date}
          </p>
        </div>
        ...
        {/* Mini bar strip */}
        <div className="flex-1 hidden sm:flex gap-1 items-end" style={{ height: 20 }}>
          {CAT_KEYS.map((k) => {
            const v = record[k] ?? 0;
            return (
              <div key={k} className="flex-1 rounded-sm" style={{
                height: `${(v / 5) * 100}%`,
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {(record.moderateExacerbations || record.seriousExacerbations) && (
            <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: "#fff0f0", color: "#b91c1c", border: "1px solid #fca5a5" }}>⚠</span>
          )}
          {record.note?.trim() && (
            <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "#f5f3ff", color: "#7c3aed", border: "1px solid #c4b5fd" }}>📝</span>
          )}
          {usedMeds.length > 0 && (
            <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "#f0f9ff", color: "#0369a1", border: "1px solid #bae6fd" }}>💊 {usedMeds.length}</span>
          )}
          {record.weight != null && (
            <span className="text-xs hidden md:inline" style={{ color: "#a0b8b6" }}>⚖ {record.weight} kg</span>
          )}
          {record.physicalActivity > 0 && (
            <span className="text-xs hidden md:inline" style={{ color: "#a0b8b6" }}>🚶 {record.physicalActivity} min</span>
          )}
        </div>

        {/* Chevron */}
        <div className="flex-shrink-0 ml-2 transition-transform" style={{ transform: expanded ? "rotate(180deg)" : "rotate(0deg)", color: "#a0b8b6" }}>
          ▾
        </div>
      </button>
      ...
      {expanded && (
        ...
        >
            ...
            <div className="space-y-2">
              {CAT_KEYS.map((k) => (
                <div key={k} className="flex items-center gap-2">
                  <span className="text-xs shrink-0" style={{ color: "#7a9a98", width: 120 }}>{t[k]}</span>
                  <ScoreBar value={record[k] ?? 0} />
                </div>
              ))}
            </div>
            <div className="flex items-center justify-between mt-3 pt-2" style={{ borderTop: "1px solid rgba(38,142,134,0.1)" }}>
              <span className="text-xs font-semibold" style={{ color: "#7a9a98" }}>Total</span>
              <span className="text-sm font-black" style={{ color: c.text }}>{record.cat8} / 40</span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {/* Exacerbation */}
            {(record.moderateExacerbations || record.seriousExacerbations) && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: "#fff0f0", border: "1px solid #fca5a5" }}>
                <span>⚠️</span>
            ...
            {usedMeds.length > 0 && (
              <div>
                <p className="text-xs font-semibold tracking-widest uppercase mb-2" style={{ color: "#7a9a98" }}>{t.usedMedicines}</p>
                <div className="space-y-1.5">
                  {usedMeds.map((m) => (
                    <div key={m.id} className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: "#f0f9ff", border: "1px solid #bae6fd" }}>
                      {m.image && (
                        <img src={m.image} alt={m.name} className="w-7 h-7 object-contain rounded-lg" style={{ background: "rgba(38,142,134,0.07)", padding: 2 }} />
                      )}
                      <span className="text-xs font-semibold flex-1" style={{ color: "#1a3a38" }}>{m.name}</span>
                      {m.times != null && (
                        <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "#e0f2fe", color: "#0369a1" }}>{m.times}{t.timesUsed}</span>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {record.note?.trim() && (
              <div>
                <p className="text-xs font-semibold tracking-widest uppercase mb-2" style={{ color: "#7a9a98" }}>{t.note}</p>
                <div className="px-3 py-2.5 rounded-xl" style={{ background: "#f5f3ff", border: "1px solid #c4b5fd" }}>
    ...
    >
      ...
      <div className="px-6 pt-6 pb-2 max-w-3xl mx-auto w-full">
        <input
          ...
          onFocus={(e) => { e.target.style.borderColor = "#268E86"; e.target.style.boxShadow = "0 0 0 3px rgba(38,142,134,0.1)"; }}
          onBlur={(e) => { e.target.style.borderColor = "rgba(38,142,134,0.2)"; e.target.style.boxShadow = "none"; }}
        />
      </div>
